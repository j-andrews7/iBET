...
.make_heatmap <- function(mat, res, anno, abundance.col.func, lfc.col.func, sig.term,
    ...
    l <- res[[sig.term]] <= sig.thresh & res[[abundance.col]] >= base_mean & abs(res[[lfc.col]]) >= log2fc
    l[is.na(l)] <- FALSE
...
.make_maplot <- function(res, ylim, fc.thresh, fc.lines, h.id, sig.term, lfc.col, abundance.col,
    ...
    if (!is.null(highlight.data)) {
        if (loess.genesets) {
            fig <- fig %>% add_lines(
                data = highlight.data, x = ~ log10(x), y = ~ fitted(loess(y ~ -log10(x), span = loess.genesets.span)),
    ...
    if (!is.null(gs)) {
        fig <- fig %>%
            ...
            add_annotations(
                x = gs$x, y = gs$y, text = gs$customdata,
                font = list(size = label.size, family = "Arial"), arrowside = "none"
            )
    } else {
        fig <- fig %>% layout(
