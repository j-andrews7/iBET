...
You can pass a DESeqDataSet directly to `shinyDE`:
...
# Pass DESeqDataSet directly - it will extract VST counts and metadata automatically
shinyDE(dds, res = res, genesets = hs.msig, swap.rownames = "SYMBOLS", annot.by = c("cell", "dex"))
```

For using results from edgeR (including direct DGEList usage), limma, or other DE tools, see `?shinyDE` for details on customizing column names.
