...
You can pass a DESeqDataSet directly to `shinyDE` with multiple comparisons:
...
# You can also specify a different transformation
# shinyDE(dds, res = res, assay = "rlog", genesets = hs.msig, swap.rownames = "SYMBOLS", annot.by = c("cell", "dex"))
```

For using results from edgeR (including direct DGEList usage), limma, or other DE tools, see `?shinyDE` for details on customizing column names and transformation options.
