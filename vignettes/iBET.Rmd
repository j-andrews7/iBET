...
# Swap ensemble IDs to gene symbols.
ens <- rownames(airway)
...
shinyDESeq2(dds, annot.by = c("cell", "dex"))
```
...
contr1 <- makeContrasts(N61311 - N052611, levels = design)
contr2 <- makeContrasts(N080611 - N052611, levels = design)
contr3<- makeContrasts(N61311 - N080611, levels = design)
contr4 <- makeContrasts(N080611 - N61311, levels = design)
...
qlf <- glmQLFTest(fit, contrast = contr1)
edgeR.res1 <- topTags(qlf, n = nrow(dge))
qlf <- glmQLFTest(fit, contrast = contr2)
edgeR.res2 <- topTags(qlf, n = nrow(dge))
qlf <- glmQLFTest(fit, contrast = contr3)
edgeR.res3 <- topTags(qlf, n = nrow(dge))
qlf <- glmQLFTest(fit, contrast = contr4)
edgeR.res4 <- topTags(qlf, n = nrow(dge))
```
...
```{r}
res <- list("comp1" = as.data.frame(edgeR.res1), "comp2" = as.data.frame(edgeR.res2), 
            "comp3" = as.data.frame(edgeR.res3), "comp4" = as.data.frame(edgeR.res4))
shinyDECorr(res, height = 800)
```

## Aesthetics and Convenience Functions

Plots are fully customizable from within the widget. Movable gene labels can be added to points by clicking on them. Specific user-defined gene sets can be highlighted within plots easily. 

### Gene set labeling

Gene sets can be passed as a named list to highlight said genes on plots.
